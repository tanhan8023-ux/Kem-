...
import { ChevronLeft, Heart, MessageCircle, Share2, Plus, User, Home, Search, Bookmark, ArrowLeft, Camera, Image as ImageIcon, X, Send, Compass } from 'lucide-react';
import { XHSPost, Persona, UserProfile, ApiSettings, WorldbookSettings } from '../types';
...
export function XHSScreen({ 
  ...
  const [activeTab, setActiveTab] = useState<'square' | 'messages' | 'me'>('square');
  ...
  const handleSendPrivateMessage = async () => {
    ...
    if (persona) {
      setIsTyping(true);
      try {
        const history = privateChats[activeChatAuthorId] || [];
